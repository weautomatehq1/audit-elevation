...
- **Symlink vs copy** for the shared prompt template (`~/.claude/skills/codex-review/review-prompt.md` ↔ `IFleet/src/pipeline/prompts/codex-review.md`)? Symlinks break on Windows checkouts; copy needs a CI check for drift. **Default: copy + CI drift check (see decisions table). Sebastian can override.**
...
- Channel name: `#ifleet-proposals` (per `plan.md`) or `#ifleet` (existing brief channel, `1504120127791042631`)? Suggest: new `#ifleet-proposals` so morning proposals don't get lost in operational chatter. **Default: reuse `#ifleet` (see decisions table below). Sebastian can override.**
