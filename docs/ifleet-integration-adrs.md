...
- `audit.finding.regressed` — payload: `{ findingId, regressionFingerprint, newOccurrenceTaskId, regressionOf, regressionClosingPr }`. Emitted by the M4 regression detector (T3 builds the standalone version tonight; M4 wires the IFleet version). Note: `regressionOf` (original finding id) and `regressionClosingPr` (PR number of the original fix, mirroring `closingPr` from the prior `audit.finding.closed` event) are the camelCase trace-event equivalents of the snake_case scan-level fields `regression_of` / `regression_closing_pr` written to `.audits/*.json` by self-heal-pipeline.md — both layers track the same data, different naming conventions per their respective contexts (TypeScript event vs. JSON file).
