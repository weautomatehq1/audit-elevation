...
| **D5** | What does the worker do when picking up an approved plan? | (a) Sebastian copies into a fresh splittasks invocation by hand. (b) Slash command `/proposer-accept` auto-builds the splittasks dir from the state file. | **(a) for v1** (paste-box by hand). Automate to (b) in v1.5 once format settles. |
